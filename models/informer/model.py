...
class BaseInformer(nn.Module):
    def __init__(self, enc_in, dec_in, c_out, out_len, factor=5, d_model=512,
                 n_heads=8, num_encoder_layers=3, num_decoder_layers=2, d_ff=512, dropout=0.0,
                 attention_type='prob', embedding_type='fixed', frequency='h', activation='gelu',
                 output_attention=False, distil=True, **kwargs):
        ...
        self.decoder = Decoder(
            [
                DecoderLayer(
                    AttentionLayer(Attention(True, factor, attention_dropout=dropout, output_attention=False),
                                   d_model, n_heads),
    ...
    def add_model_specific_arguments(parent_parser):
        ...
        parser.add_argument('--no_distil', action='store_true',
        ...
        parser.add_argument('--attention', '--attn', type=str, default='prob', choices=['prob', 'full'],
                            help='Type of attention used in the encoder')
        parser.add_argument('--embedding_type', '--embed', type=str, default='timefeature', 
                            choices=['timefeature', 'fixed', 'learned'],
                            help='Type of time features encoding')
        parser.add_argument('--activation', type=str, default='gelu', help='Activation function')
        parser.add_argument('--output_attention', action='store_true', help='Whether to output attention in the encoder')
...
class Informer(BaseInformer):
    def __init__(self, enc_in, dec_in, c_out, out_len, factor=5, d_model=512,
        ...
        super(Informer, self).__init__(
            enc_in, dec_in, c_out, out_len, factor=factor, d_model=d_model,
            n_heads=n_heads, num_encoder_layers=num_encoder_layers, num_decoder_layers=num_decoder_layers, 
            d_ff=d_ff, dropout=dropout, attention_type=attention_type, embedding_type=embedding_type, 
            frequency=frequency, activation=activation, output_attention=output_attention, distil=distil
        )
        Attention = ProbSparseAttention if attention_type == 'prob' else FullAttention
        self.encoder = Encoder(
            [
                EncoderLayer(
                    AttentionLayer(Attention(False, factor, attention_dropout=dropout, 
            ...
            [
                SelfAttentionDistil(d_model) for l in range(num_encoder_layers - 1)
            ] if distil else None,
            nn.LayerNorm(d_model)
        )
    ...
    def __init__(self, enc_in, dec_in, c_out, out_len, factor=5, d_model=512,
        ...
        encoders = [
            Encoder(
                ...
                [
                    SelfAttentionDistil(d_model) for l in range(el - 1)
                ] if distil else None,
                nn.LayerNorm(d_model)
            ) for el in stacks
        ]
